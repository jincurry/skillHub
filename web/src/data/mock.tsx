...
export type CheckKind = 'ok' | 'warn' | 'err';
export interface Draft {
  ...
  updated: string;
  summary: string;
  checks: { kind: CheckKind; label: string }[];
  canSubmit: boolean;
}

export const MY_DRAFTS: Draft[] = [
  {
    name: 'go-code-review',
...
export interface MySkill {
  ...
  iconClass: string;
  version: string;
  status: 'published' | 'draft' | 'review' | 'yanked' | 'deprecated';
  activations: number;
  delta: number;
  updated: string;
}
...
export interface Notification {
  ...
  type: string;
  unread: boolean;
  icon: 'review' | 'comment' | 'publish' | 'warn';
  text: ReactNode;
  time: string;
}
...
export interface Skill {
  ...
  iconClass: string;
  version: string;
  status: 'published' | 'review' | 'deprecated' | 'yanked' | 'draft';
  classification: 'L1' | 'L2' | 'L3';
  rating: number;
  ratings: number;
  activations: number;
...
export interface Review {
  ...
  submitted: string;
  sla: string;
  urgency: 'overdue' | 'soon' | 'ok' | 'done' | 'rejected';
  reviewers: string[];
...
export const SKILL_PROFILES: Record<string, {
  icon: string; iconClass: string; level: 'L1' | 'L2' | 'L3'; hot: boolean;
  desc: string; rating: string; reviews: number; weekly: string; users: number;
  maintainer: string; updated: string; version: string; versions: number;
}> = {
  'go-code-review': {
