...
export function DownloadMenu({ ns, name, version }: Props) {
  ...
  async function doDownload() {
    try {
      ...
      window.alert('下载失败: ' + (e as Error).message);
    }
  }
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', display: 'inline-flex' }}>
      <button
        className="btn"
        onClick={doDownload}
        title={`下载 ${ref}${version ? ` v${version}` : ''} 的文件 bundle (.tar.gz)`}
      ...
      {open && (
        ...
        >
          <div style={{ fontWeight: 600, marginBottom: 6, color: 'var(--text)' }}>下载 Bundle</div>
          <button
            className="btn sm"
            onClick={() => { setOpen(false); void doDownload(); }}
            style={{ width: '100%', justifyContent: 'center' }}
          >
            <IconDownload size={13} /> 浏览器下载 .tar.gz
          </button>

          <div style={{ borderTop: '1px solid var(--border)', margin: '12px 0 10px' }} />

          <div style={{ fontWeight: 600, marginBottom: 6, color: 'var(--text)' }}>使用 CLI</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, lineHeight: 1.5 }}>
            已安装 <code style={{ background: 'var(--bg-soft)', padding: '1px 4px', borderRadius: 3 }}>skillhub</code> CLI 后，复制以下命令到终端运行：
          </div>

          <CmdRow label="拉取到当前目录" cmd={pullCmd} copied={copied === 'pull'} onCopy={() => copy(pullCmd, 'pull')} />
          <CmdRow label="查看 skill 详情" cmd={getCmd} copied={copied === 'get'} onCopy={() => copy(getCmd, 'get')} />
          <CmdRow label="记录一次激活" cmd={activateCmd} copied={copied === 'activate'} onCopy={() => copy(activateCmd, 'activate')} />

          <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 10, lineHeight: 1.5 }}>
            未安装 CLI？参考 <code style={{ background: 'var(--bg-soft)', padding: '1px 4px', borderRadius: 3 }}>README</code> 中的「CLI」章节。
          </div>
        </div>
      )}
    </div>
