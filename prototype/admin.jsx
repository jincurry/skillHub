  ...
  return (
    <div className="content-inner">
      ...
      <div className="tabs">
        ...
        <div className={`tab ${tab==="namespaces"?"active":""}`} onClick={()=>setTab("namespaces")}>命名空间 <span className="count">7</span></div>
      ...
      {tab === "overview" && (
        <div>
          <div className="stat-strip">
            <div className="stat"><div className="stat-label">活跃 Skills</div><div><span className="stat-value num">87</span><span className="stat-delta up"><IconArrowUp size={11}/>4</span></div></div>
            ...
            <div className="stat"><div className="stat-label">本月调用</div><div><span className="stat-value num">487K</span><span className="stat-delta up"><IconArrowUp size={11}/>12.4%</span></div></div>
              ...
              <div className="card-body">
                {[
                  ...
                  <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom: i<4?"1px solid var(--border)":"none"}}>
                    <span style={{width:8,height:8,borderRadius:"50%",background: s.status==="ok"?"var(--green)":"var(--amber)",boxShadow: `0 0 0 3px color-mix(in oklab, ${s.status==="ok"?"var(--green)":"var(--amber)"} 20%, transparent)`}}/>
      ...
      {tab === "namespaces" && (
        <div className="card">
          ...
          <div className="card-body flush table-wrap">
            <table className="tbl">
              <thead><tr><th>命名空间</th><th>Owner</th><th style={{textAlign:"right"}}>Skills</th><th style={{textAlign:"right"}}>成员</th><th>默认密级</th><th>创建时间</th><th></th></tr></thead>
              <tbody>
                {NAMESPACES.map((ns,i)=>(
                  <tr key={ns.id}>
                    <td><span className="mono" style={{fontWeight:600}}>{ns.id}</span></td>
                    <td><span className="mono">@{["alice","frank","ivan","george","judy","henry","diana"][i] || "alice"}</span></td>
                    <td className="num" style={{textAlign:"right",fontWeight:500}}>{ns.count}</td>
                    <td className="num" style={{textAlign:"right"}}>{4 + i*2}</td>
                    <td><ClassificationTag level={i % 3 === 2 ? "L3" : i % 3 === 1 ? "L1" : "L2"}/></td>
                    <td style={{fontSize:12,color:"var(--text-subtle)"}}>2024-{String((i%12)+1).padStart(2,"0")}-{String((i*3)%28+1).padStart(2,"0")}</td>
      ...
      {tab === "policies" && (
        <div style={{maxWidth:760}}>
          <div className="card" style={{marginBottom:"var(--gap)"}}>
            ...
            <div className="card-body" style={{display:"flex",flexDirection:"column",gap:14}}>
              {[
                ...
                {n:"PR 必须关联 issue", d:"发布前要求关联 Jira / Linear issue",on:false},
          ...
          <div className="card">
            <div className="card-header"><h3 className="card-title">SLA 配置</h3></div>
            <div className="card-body" style={{display:"flex",flexDirection:"column",gap:12}}>
              {[
                {l:"L1 公开 — 审批超时",v:"24 小时"},
                {l:"L2 内部 — 审批超时",v:"72 小时"},
                {l:"L3 敏感 — 审批超时",v:"96 小时"},
                {l:"超时升级到",v:"namespace owner"},
              ].map((s,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:13,paddingBottom:i<3?10:0,borderBottom:i<3?"1px solid var(--border)":"none"}}>
                  <span>{s.l}</span>
